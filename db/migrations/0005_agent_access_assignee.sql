-- 011-agent-access-mcp. HAND-EDITED: work_items_assignee_member_fk uses
-- `ON DELETE SET NULL ("assignee_user_id")` (Postgres 15+), which drizzle-kit
-- can't generate — a plain SET NULL would also null the NOT NULL project_id.
-- See db/schema.ts and specs/011-agent-access-mcp/data-model.md.
ALTER TYPE "public"."notification_type" ADD VALUE 'work_item_assigned';--> statement-breakpoint
CREATE TABLE "agent_last_used" (
	"user_id" text NOT NULL,
	"client_id" text NOT NULL,
	"last_used_at" timestamp with time zone NOT NULL,
	CONSTRAINT "agent_last_used_user_id_client_id_pk" PRIMARY KEY("user_id","client_id")
);
...
--> statement-breakpoint
ALTER TABLE "work_item_activity" ADD COLUMN "actor_user_id" text;--> statement-breakpoint
ALTER TABLE "work_item_activity" ADD COLUMN "agent_client_id" text;--> statement-breakpoint
ALTER TABLE "work_item_activity" ADD COLUMN "agent_name" text;--> statement-breakpoint
ALTER TABLE "work_items" ADD COLUMN "assignee_user_id" text;--> statement-breakpoint
...
ALTER TABLE "work_items" ADD CONSTRAINT "work_items_assignee_member_fk" FOREIGN KEY ("project_id","assignee_user_id") REFERENCES "public"."project_members"("project_id","user_id") ON DELETE SET NULL ("assignee_user_id") ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "work_items_assignee_idx" ON "work_items" USING btree ("project_id","assignee_user_id");