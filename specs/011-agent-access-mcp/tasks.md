...
- [x] T001 Actualizar dependencias en `package.json`:
  - `better-auth` a `^1.7.5`.
...
- [x] T003 Crear `lib/actor.ts` (solo servidor, **sin** `"use server"`), según contracts/app-changes.md § Módulo `lib/actor.ts`:
...
- [x] T004 [P] Tests de `lib/actor.ts` en `tests/unit/actor.test.ts`, mockeando `@/lib/auth`:
...
- [x] T005 Modificar `lib/permissions.ts`:
...
- [x] T006 Modificar `db/schema.ts` según data-model.md:
  - `work_items`:
    - Quitar `stakeholder`.
...
- [x] T007 Configurar Better Auth en `lib/auth.ts`:
...
- [x] T008 Regenerar el esquema de Better Auth con `npm run auth:generate`, que actualiza `db/auth-schema.ts` con `jwks` y `oauth_*`. Después, `npm run db:generate`.
...
- [x] T009 Crear `lib/activity.ts` (solo servidor): `logActivity(tx, workItemId, type, payload)`, que inserta en `work_item_activity` con `actorUserId`, `agentClientId` y `agentName` tomados de `getActor()`.
...
- [x] T010 [P] Test `tests/unit/activity-logging.test.ts`: recorre `lib/**/*.ts` y falla si alguna aparición de `insert(workItemActivity)` está fuera de `lib/activity.ts`.
- [x] T011 Quitar `stakeholder` de todo el código:
...
- [x] T012 [P] [F11-US1] En `tests/unit/work-items.test.ts`, casos de `updateWorkItem` con `assigneeUserId`:
...
- [x] T013 [P] [F11-US1] En `tests/unit/work-item-view.test.ts`:
...
- [x] T014 [P] [F11-US1] Tests de salida de un miembro en `tests/unit/projects.test.ts`: `removeMember` y `leaveProject` registran `assignee_changed` con `reason: "member_left"` por cada Work Item asignado antes de borrar la membresía, todo en una transacción, y no notifican.

### Implementation

- [x] T015 [F11-US1] Crear `lib/work-item-assignee.ts` (solo servidor):
...
- [x] T016 [F11-US1] Modificar `updateWorkItem` en `lib/actions/work-items.ts`:
...
- [x] T017 [F11-US1] Modificar `removeMember` y `leaveProject` en `lib/actions/projects.ts`: envolver cada una en `db.transaction` y llamar `unassignOnMemberExitWithinTx` antes del `DELETE` de `project_members`.
- [x] T018 [F11-US1] Lecturas con asignado:
...
- [x] T019 [P] [F11-US1] Crear `components/ui/avatar.tsx`: foto (`user.image`) o iniciales, tamaños `sm` y `md`, con `title` y `aria-label` con el nombre.
- [x] T020 [F11-US1] Crear `components/work-items/AssigneePicker.tsx`:
...
- [x] T021 [F11-US1] Modificar `components/work-items/WorkItemDetailView.tsx`:
...
- [x] T022 [P] [F11-US1] Modificar `components/board/WorkItemCard.tsx` (y el tipo que recibe desde `Board.tsx` / `StageColumn.tsx`): avatar `sm` del asignado. El `aria-label` agrega ", assigned to X".
- [x] T023 [F11-US1] Modificar `lib/work-item-view.ts`, `components/views/ViewFilters.tsx`, `components/views/WorkItemsTable.tsx` y `components/views/WorkItemsList.tsx`:
...
- [x] T024 [F11-US1] Notificaciones en `lib/actions/accounts-invitations.ts`:
...
- [x] T025 [F11-US1] Modificar `components/sidebar/NotificationsPanelClient.tsx`:
...
- [x] T026 [F11-US1] Registrar `markNotificationRead` en `tests/unit/action-permissions.test.ts` como acción de cuenta. Ajustar el mock si hace falta.
- [x] T027 [F11-US1] E2E `tests/e2e/assignee.spec.ts` (quickstart § 1):
...
- [x] T028 [P] [F11-US2] `tests/unit/mcp-route.test.ts`, que prueba el handler exportado por `app/api/mcp/route.ts` con `@/lib/auth` y `@/db/client` mockeados:
...
- [x] T029 [F11-US2] Crear `lib/mcp/result.ts`:
...
- [x] T030 [F11-US2] Crear `lib/mcp/rate-limit.ts`: token bucket en memoria por `${userId}:${clientId}`, 120 por minuto y ráfagas de 30, con limpieza perezosa de entradas viejas. `consume(key)` devuelve `{ ok: true }` o `{ ok: false, retryAfterSeconds }`.
- [x] T031 [F11-US2] Crear `lib/mcp/server.ts`:
...
- [x] T032 [F11-US2] Crear `app/api/mcp/route.ts`, que exporta solo `POST`:
...
- [x] T033 [F11-US2] Rutas de descubrimiento, si la verificación de T002 muestra que hacen falta en la raíz:
...
- [x] T034 [F11-US2] Crear la pantalla de consentimiento `app/(auth)/consent/page.tsx` (Server Component) + `components/auth/ConsentForm.tsx` (cliente):
...
- [x] T035 [F11-US2] `lib/auth-client.ts`: agregar `oauthProviderClient()`, de `@better-auth/oauth-provider/client` o el export equivalente de `@better-auth/mcp`. Confirmar que `/sign-in` (email y Google) reanuda `/oauth2/authorize` tras iniciar sesión: si el plugin no lo hace solo, pasar `callbackURL` con la query firmada.
- [x] T036 [F11-US2] Verificación manual local de quickstart § 2, pasos 5-6: `curl` sin token → 401 + `WWW-Authenticate`, y metadatos `.well-known` accesibles. Registrar en research.md qué rutas se usaron.
...
- [x] T037 [P] [F11-US3] `tests/unit/mcp-search.test.ts` para `lib/mcp/search.ts` (puro):
...
- [x] T038 [P] [F11-US3] `tests/unit/mcp-resolve.test.ts` para `lib/mcp/resolve.ts`:
...
- [x] T039 [F11-US3] Crear `lib/mcp/resolve.ts`:
  ...
  - `parseDisplayId`.
- [x] T040 [F11-US3] Crear `lib/mcp/search.ts` (puro): `searchRows(rows, { text, assigneeIds, columnIds, state, currentUserId, limit, offset })`.
- [x] T041 [F11-US3] Crear `lib/mcp/tools/read.ts` con las cinco herramientas de contracts/mcp-tools.md § Lectura, todas con `readOnlyHint: true` y `openWorldHint: false`:
...
- [x] T042 [P] [F11-US4] En `tests/unit/work-items.test.ts`, casos de `createWorkItems`:
...
- [x] T043 [P] [F11-US4] `tests/unit/mcp-tools.test.ts`:
  - La lista exacta de las 16 herramientas registradas coincide con contracts/mcp-tools.md.
...
- [x] T044 [F11-US4] Crear `lib/work-item-create.ts` (solo servidor): `createWorkItemsInStage(tx, { project, stage, items })`:
...
- [x] T045 [F11-US4] En `lib/actions/work-items.ts`:
...
- [x] T046 [F11-US4] Crear `lib/mcp/tools/work-items.ts`: `create_work_items`, `update_work_item`, `move_work_item`, `set_parent`, `link_related` y `delete_work_item`, según contracts/mcp-tools.md § Escritura.
...
- [x] T047 [F11-US4] Crear `lib/mcp/tools/columns.ts`: `create_column`, `rename_column`, `reorder_columns`, `set_column_closing` y `delete_column`, llamando a `createStage`, `renameStage`, `reorderStages`, `setStageClosing` y `deleteStage`. `delete_column` lleva `destructiveHint: true`.
- [x] T048 [F11-US4] Verificar que `revalidatePath` dentro de las Server Actions no falla cuando se llama desde el Route Handler MCP. Si falla, envolverlo en el helper `safeRevalidate` de `lib/`, que lo ignora fuera de un contexto de render.
...
- [x] T049 [F11-US5] Crear `lib/actions/agents.ts` (`"use server"`), según contracts/app-changes.md:
...
- [x] T050 [P] [F11-US5] Crear `components/settings/ConnectedAgentsList.tsx` (cliente):
...
- [x] T051 [F11-US5] Crear `app/(workspace)/settings/agents/page.tsx`, que usa `listConnectedAgents`, y agregar el enlace "Connected agents" en la barra lateral (`components/sidebar/ProjectSidebarClient.tsx`, zona de cuenta).
- [x] T052 [F11-US5] E2E `tests/e2e/connected-agents.spec.ts`:
...
- [x] T054 [P] Actualizar `AGENTS.md` § Current state: 011 implementado, `lib/actor.ts` + `lib/activity.ts` (regla: toda actividad pasa por `logActivity`; `runAsAgent` solo en `app/api/mcp/route.ts`), `lib/mcp/` y la regla de que las herramientas MCP solo llaman Server Actions.
- [x] T055 Actualizar `app/(legal)/privacy/page.tsx`, si corresponde: la instancia guarda autorizaciones de agentes y quién hizo cada cambio.
- [x] T056 Correr `npm run lint`, `npx tsc --noEmit`, `npm test`, `npm run build` y `npm run test:e2e` (base `dev`, desechable). Corregir lo que falle.
...
- T008: `npm run auth:generate` usa la CLI `auth@1.7.5` con `db/auth-schema.config.ts`, porque `@better-auth/cli` quedó obsoleta y la CLI nueva no resuelve el alias `@/`. Los plugins viven en `lib/auth-plugins.ts`. Hay dos migraciones: `0005` (aditiva, con la FK editada a mano) y `0006` (elimina `stakeholder`). Así se evita la pregunta interactiva de rename de drizzle-kit (research.md § Hallazgos de implementación).
- T015: el helper de salida se llama `logUnassignOnMemberExitWithinTx`: solo registra el evento, y la FK hace el `SET NULL`.
- T044: la creación en bloque vive en `lib/actions/work-items.ts` como Server Action `createWorkItems` más un helper **no exportado**, `applyWorkItemFieldsWithinTx`, compartido con `updateWorkItem`. No hay un `lib/work-item-create.ts` aparte: mismo archivo, mismas reglas, y nada queda expuesto.
- T028: `createMcpRoute({ verify })` vive en `lib/mcp/route-handler.ts`, porque un archivo `route.ts` de Next.js solo puede exportar los handlers HTTP.
- T033: las tres rutas `.well-known` (protected-resource, authorization-server y openid-configuration) sí hacen falta.
- T048: `revalidatePath` funciona dentro del Route Handler MCP; no hizo falta un envoltorio.
- Extra: `lib/work-item-activity.ts` (descripción pura del historial, compartida por el detalle y `get_work_item`), `lib/mcp/last-used.ts` y `tests/unit/mcp-rate-limit.test.ts`.
