...
- [X] T056 Correr `npm run test:e2e` **contra una rama desechable de Neon** (nunca datos reales — `tests/e2e/setup.ts` trunca todas las tablas); confirmar que las suites de Fase 1 y de 005/006 siguen en verde (SC-007: los flujos de Owner y Miembro no cambian) y que pasa `tests/e2e/roles-permissions.spec.ts` — *Hecho 2026-09-22*: 43/43 en verde. La suite de Fase 1 ya fallaba antes de 007 (15 de 23 en el commit `1b38f1a`); se corrigieron los tests (hidratación, selectores ambiguos, recargas antes de que terminara la Server Action) y cuatro bugs reales de la app — ver el mensaje del commit.
