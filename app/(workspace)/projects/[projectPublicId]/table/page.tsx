...
import { WorkItemsTable } from "@/components/views/WorkItemsTable";

// The Table view of 009-work-item-views. Same server-side resolution as the board
...
export default async function ProjectTablePage({ params }: { params: Promise<{ projectPublicId: string }> }) {
  ...
  const { rows, options, role, currentUserId } = result.data;
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      {/* useSearchParams inside: wrapped per the Next.js guide (use-search-params.md § Prerendering). */}
      <Suspense>
        <ProjectViewHeader projectPublicId={projectPublicId} role={role} active="table" />
        {options.stages.length === 0 || rows.length === 0 ? (
          ...
          <WorkItemsTable projectPublicId={projectPublicId} rows={rows} options={options} currentUserId={currentUserId} />
        )}
      </Suspense>
    </div>
  );
