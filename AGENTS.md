...
3. [`specs/`](specs/) — one directory per feature, numbered (`001-accounts-invitations` … `004-work-items` for Phase 1, then `005-work-item-relationships`, `006-work-item-detail-view`, `007-roles-permissions` for Phase 2, and `008-work-item-fields`, `009-work-item-views` for Phase 3, plus `010-legal-pages` — the public `/privacy` and `/terms` pages, a deployment requirement outside the phase roadmap — and `011-agent-access-mcp` plus `012-work-item-discussion` for Phase 4). Each feature directory can contain:
...
Phases 1 and 2 are implemented (`001`-`007`) and merged to `main`. Phase 3 is confirmed as extended fields plus list and table views (no calendar yet): `008-work-item-fields` and `009-work-item-views` (List and Table views) are implemented and merged to `main` — Phase 3 is complete except the manual quickstart passes (T052 of 008, T028 of 009). Phase 4 so far is `011-agent-access-mcp` (AI agents over MCP + the Work Item Assignee that replaced `stakeholder`) and `012-work-item-discussion` (redesigned Work Item detail, comments, estimate and time entries, History tab), both merged to `main` and deployed (production migrated through `0007`); any other Phase 4 candidate (`README.md` § Roadmap) must still go through `/speckit-specify` before any code. Branching: feature branches (`NNN-feature-name`, or `codex/NNN-feature-name` when another agent cut it) are cut from `main` and merged back into `main` when done (no separate `dev` branch). The first instance is deployed on Render against a **production** Neon branch (`main`, set only in Render's environment); local development and e2e use a separate `dev` branch in `.env.local`. Migrations must be applied to both branches — `dev` first, then production before pushing the code that needs them: Render auto-deploys `main` on push and never runs migrations, so pushing first crashes every page that reads a new column (this happened with `0007`). Still confirm before running e2e, which truncates whatever `.env.local` points at. The source follows the layout in `specs/001-accounts-invitations/plan.md` § Project Structure: routes in `app/`, the Drizzle schema and migrations in `db/`, Server Actions in `lib/actions/` (every export of those `"use server"` files is a public endpoint), plain server-only helpers in `lib/` (`permissions.ts`, `work-item-queries.ts`, `work-item-catalogs.ts`, `invitations.ts`), the actor and audit plumbing of 011 (`actor.ts` — who the request acts as: the session user, or an AI agent's user inside `app/api/mcp/route.ts`; `activity.ts` — `logActivity`, the only way to write `work_item_activity`; `work-item-assignee.ts`), 012's helpers (`work-item-time.ts` — hours ⇄ minutes; comments and time entries live in `lib/actions/work-item-discussion.ts`), the MCP server in `lib/mcp/` (its tools only resolve public ids and call the exported Server Actions — never `db` for writes), pure modules shared with the client (`roles.ts`, `work-item-fields.ts`, `work-item-closing.ts`, `work-item-view.ts` — the List/Table sort, filter, tree and URL rules; `legal.ts` — the instance operator shown on `/privacy` and `/terms`), UI in `components/`, and Vitest/Playwright suites in `tests/unit/` and `tests/e2e/`. Check `README.md` § Project Status for the up-to-date picture.
...
- Every write to `work_item_activity` goes through `logActivity` (`lib/activity.ts`), which stamps who made the change and through which AI agent (FR-034 of 011). `tests/unit/activity-logging.test.ts` fails on any direct insert.
- Permission helpers read the caller from `getActor()` (`lib/actor.ts`), never `getSession()` directly: that's what lets the MCP route (`runAsAgent`, only called in `lib/mcp/route-handler.ts` after verifying the token and the user's consent) reuse every Server Action unchanged. Account-level actions (notifications, connected agents, creating projects) still require a browser session — agents must not reach them. New MCP tools must stay out of project/member administration (FR-023 of 011), and `tests/unit/mcp-route.test.ts` pins the exact tool list.
- A Work Item's assignee must be a current member of its project — enforced by a composite FK with `ON DELETE SET NULL ("assignee_user_id")`, hand-edited into migration `0005` (drizzle can't express the column list). Assign through `setAssigneeWithinTx` (`lib/work-item-assignee.ts`), which also logs and notifies.
